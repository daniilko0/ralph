...
for event in bot.longpoll.listen():
    bot.event = event
    pprint(bot.event.raw)
    payload = json.loads(bot.event.payload)
    print(payload)
    if bot.event.type == bot.NEW_MESSAGE and bot.event.object.text and bot.event.object.out == 0 and \
        ...
        if text == 'начать':
            ...
            bot.send_message(msg='Отправка клавиатуры.', pid=bot.event.object.from_id,
                             keyboard=open(f'keyboards/names/{payload["button"]}.json', 'r', encoding='UTF-8').read())
            ...
            bot.send_message(msg='Выберите статью расходов (колонку в таблице)', pid=bot.event.object.from_id,
            ...
            bot.send_message(msg='Отправка клавиатуры с алфавитом.', pid=bot.event.object.from_id,
